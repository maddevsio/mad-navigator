  ...
  while (*format) {
    ...
    switch (type) {
      ...
      case 'D': { // Date (int, int, int), -1 if empty.
        kov_date_t *date = va_arg(ap, kov_date_t *);
      ...
      case 'T': { // Time (int, int, int, int), -1 if empty.
        kov_time_t *time_ = va_arg(ap, kov_time_t *);
...
kov_timestamp_t nmea_gettime(const kov_date_t *date,
                             const kov_time_t *time_) {  
  return RTC_GetTimestampDateTime(date, time_);
}
///////////////////////////////////////////////////////

inline int_least32_t nmea_rescale(struct nmea_float *f, int_least32_t new_scale) {
  if (f->scale == 0)
    return 0;
...
inline float nmea_tofloat(const struct nmea_float *f) {
  if (f->scale == 0)
    return NAN;
...
inline float nmea_tocoord(const struct nmea_float *f) {
  if (f->scale == 0)
    return NAN;
